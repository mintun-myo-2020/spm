...
export function CreateScheduleForm({ classId, onSuccess, onCancel }: Props) {
  const { showToast } = useToast();
  const recurringForm = useForm<RecurringForm>({ resolver: zodResolver(recurringSchema), defaultValues: { type: 'recurring', dayOfWeek: 1 } });
  const oneOffForm = useForm<OneOffForm>({ resolver: zodResolver(oneOffSchema), defaultValues: { type: 'one-off' } });
  ...
  return (
    <div data-testid="create-schedule-form">
      <div className="mb-4 flex gap-2">
        <Button size="sm" color={!isOneOff ? 'blue' : 'gray'} onClick={() => setIsOneOff(false)} data-testid="tab-recurring">Weekly Recurring</Button>
        <Button size="sm" color={isOneOff ? 'blue' : 'gray'} onClick={() => setIsOneOff(true)} data-testid="tab-one-off">One-off / Makeup</Button>
      </div>

      {!isOneOff ? (
        <form onSubmit={recurringForm.handleSubmit(handleRecurring)} className="space-y-3">
          <div>
        ...
        <form onSubmit={oneOffForm.handleSubmit(handleOneOff)} className="space-y-3">
