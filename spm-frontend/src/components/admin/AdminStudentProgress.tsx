...
export function AdminStudentProgress() {
  ...
  return (
    <div data-testid="admin-student-progress">
      <PageHeader title={`Progress: ${overall.studentName}`} subtitle={`Average: ${overall.averageScore.toFixed(1)}${overall.improvementVelocity ? ` · Improvement: ${overall.improvementVelocity.improvement >= 0 ? '+' : ''}${overall.improvementVelocity.improvement.toFixed(1)}` : ''}`} backTo={`/admin/classes/${classId}/students/${studentId}`} />
